...
def parse_function(expr):
    ...
    try:
        ...
        # Parse the expression to a sympy expression
        x = sp.symbols('x')
...
def calculate_derivative():
    """Calculate the derivative and display the result."""
    try:
        ...
        # Show the result on the window (instead of a popup)
...
def plot_function_and_derivative(func, x, h):
    ...
    for widget in plot_frame.winfo_children():
        widget.destroy()  # Remove any existing widgets (including the previous plot)
...
function_input.insert(0, "np.sin(x) * np.cos(x) * x**2")  # Default value
function_input.grid(row=0, column=1, padx=10, pady=5)

# x value input with default value
...
x_input.insert(0, "2")  # Default value
x_input.grid(row=1, column=1, padx=10, pady=5)

# h value input with default value
...
h_input.insert(0, "0.01")  # Default value
h_input.grid(row=2, column=1, padx=10, pady=5)

# tol value input with default value
...
tol_input.insert(0, "1e-6")  # Default value
